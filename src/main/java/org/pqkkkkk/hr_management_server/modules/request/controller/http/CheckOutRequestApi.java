...
public class CheckOutRequestApi {
    ...
    public ResponseEntity<ApiResponse<CheckOutRequestDTO>> createCheckOutRequest(
            @AuthenticationPrincipal(expression = "userId") Long employeeId,
            @Valid @RequestBody CreateCheckOutRequestRequest request) {
        // Convert DTO to entity
        Request requestEntity = request.toEntity(employeeId);
        // Gọi service createCheckOutRequest
        Request createdRequest = checkOutRequestCommandService.createCheckOutRequest(requestEntity);
        // Convert created entity to DTO
        CheckOutRequestDTO responseDTO = CheckOutRequestDTO.fromEntity(createdRequest);
        // Build response
