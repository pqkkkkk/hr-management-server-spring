 ...
 * These methods are primarily used by the Request module when approving various
 * types of requests.
 * Each method handles the specific timesheet updates required for different
 * request types.
 */
public interface TimeSheetCommandService {
     ...
     * - Calculates totalWorkCredit based on actual working hours (proportional to 9
     * hours standard)
     * </p>
     * 
     * @param employeeId   ID of the employee checking out
     * @param checkOutTime The approved check-out time from the request
     * @return Updated DailyTimeSheet with check-out information and calculated
     *         metrics
     * @throws IllegalArgumentException if employeeId is null or employee not found
     * @throws IllegalArgumentException if checkOutTime is null
     * @throws IllegalArgumentException if timesheet doesn't exist or checkInTime is
     *                                  not set
    ...
    /**
     * Handles leave request approval by creating or updating timesheets for
     * multiple dates.
     * <p>
    ...
    /**
     * Handles WFH (Work From Home) request approval by creating or updating
     * timesheets for multiple dates.
     * <p>
     * Operations performed for each WFH date:
     * - Creates or retrieves timesheet for the date
     * - Based on ShiftType:
     * - FULL_DAY: Sets morningWfh=true, afternoonWfh=true, both statuses to
     * PRESENT, workCredit=1.0
     * - MORNING: Sets morningWfh=true, morningStatus to PRESENT, workCredit=0.5
     * - AFTERNOON: Sets afternoonWfh=true, afternoonStatus to PRESENT,
     * workCredit=0.5
     ...
     * - totalWorkCredit (based on new working hours, proportional to 9 hours
     * standard)
     ...
     * @throws IllegalArgumentException if timesheet doesn't exist for the target
     *                                  date
    ...
    /**
     * Extended version of handleTimesheetUpdateApproval that allows updating
     * attendance status and WFH flags in addition to check times.
     * <p>
     * This method enables more comprehensive timesheet corrections, including:
     * - Changing attendance status (e.g., converting a LEAVE day back to PRESENT)
     * - Setting/unsetting WFH flags for specific shifts
     * - All operations from the basic version
     * </p>
     * 
     * @param employeeId         ID of the employee whose timesheet is being updated
     * @param targetDate         The date of the timesheet to update
     * @param newCheckInTime     The new check-in time (optional if only updating
     *                           status)
     * @param newCheckOutTime    The new check-out time (optional if only updating
     *                           status)
     * @param newMorningStatus   New morning attendance status (null = no change)
     * @param newAfternoonStatus New afternoon attendance status (null = no change)
     * @param newMorningWfh      New morning WFH flag (null = no change)
     * @param newAfternoonWfh    New afternoon WFH flag (null = no change)
     * @return Updated DailyTimeSheet with all changes applied
     * @throws IllegalArgumentException if validation fails
     * @throws IllegalStateException    if timesheet doesn't exist or is finalized
     */
    DailyTimeSheet handleTimesheetUpdateApproval(
            String employeeId,
            LocalDate targetDate,
