...
class ProfileHRCommandServiceIntegrationTest {
        ...
        @BeforeEach
        void setUp() {
                // Sample data is loaded from Flyway migration
                // V3__insert_profile_sample_data.sql
        ...
        // ========== Integration Tests for Department Name Lookup ==========

        @Test
        @DisplayName("Should successfully update user's department by name")
        void testUpdateProfile_WithDepartmentName_Success() {
                // Arrange
                // Marketing department from migration: d3c4d5e6-a7b8-9012-cdef-123456789012
                Department newDepartment = Department.builder()
                ...
                // Assert
                assertNotNull(updatedUser.getDepartment());
                assertEquals("d3c4d5e6-a7b8-9012-cdef-123456789012", updatedUser.getDepartment().getDepartmentId());
                assertEquals("Marketing", updatedUser.getDepartment().getDepartmentName());

                // Verify persistence
                User fetchedUser = profileDao.getProfileById(testUserId);
                assertEquals("Marketing", fetchedUser.getDepartment().getDepartmentName());
        }

        @Test
        @DisplayName("Should throw IllegalArgumentException when department name does not exist")
        void testUpdateProfile_WithInvalidDepartmentName_ThrowsException() {
                // Arrange
        ...
        @Test
        @DisplayName("Should prioritize department name over department ID when both are provided")
        void testUpdateProfile_WithBothDepartmentNameAndId_UsesName() {
                // Arrange
                // Provide fake ID but real name - should use name lookup
                Department departmentWithBoth = Department.builder()
                                .departmentId("fake-id-should-be-ignored")
                                .departmentName("Sales")
                                .build();
                User updateRequest = User.builder()
                                .userId(testUserId)
                                .department(departmentWithBoth)
                                .build();

                // Act
                User updatedUser = profileHRCommandService.updateProfile(updateRequest);

                // Assert - should have resolved to Sales department via name lookup
                assertNotNull(updatedUser.getDepartment());
                assertEquals("d4d5e6f7-b8c9-0123-def0-234567890123", updatedUser.getDepartment().getDepartmentId());
                assertEquals("Sales", updatedUser.getDepartment().getDepartmentName());
        }
}
