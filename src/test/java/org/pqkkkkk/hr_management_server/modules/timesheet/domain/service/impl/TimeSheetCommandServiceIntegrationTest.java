...
class TimeSheetCommandServiceIntegrationTest {
        ...
        @Test
        @DisplayName("Should throw exception when duplicate check-in on existing timesheet")
        void testHandleCheckInApproval_ExistingTimesheet_ThrowsException() {
                // Arrange - Date with existing timesheet with check-in from sample data
                // (2024-12-01)
                LocalDateTime newCheckInTime = LocalDateTime.of(2024, 12, 1, 8, 30);

                // Act & Assert - New validation blocks duplicate check-in
                IllegalStateException exception = assertThrows(
                                IllegalStateException.class,
                                () -> timeSheetCommandService.handleCheckInApproval(testEmployeeId, newCheckInTime));

                assertTrue(exception.getMessage().contains("already has check-in") ||
                                exception.getMessage().contains("Timesheet Update"));
        }
        ...
        void testHandleCheckOutApproval_NoCheckIn_ThrowsException() {
                ...
                // Act & Assert - Validation throws IllegalStateException
                IllegalStateException exception = assertThrows(
                                IllegalStateException.class,
                ...
                assertTrue(exception.getMessage().contains("No timesheet found") ||
                                exception.getMessage().contains("no check-in"));
        }
        ...
        @Test
        @DisplayName("Should throw exception when applying leave to already worked day")
        void testHandleLeaveApproval_ExistingTimesheet_ThrowsConflictException() {
                // Arrange - Date with existing worked timesheet (2024-12-01 has PRESENT status)
                ...
                // Act & Assert - Validation blocks leave on already worked day
                IllegalStateException exception = assertThrows(
                                IllegalStateException.class,
                                () -> timeSheetCommandService.handleLeaveApproval(
                                                testEmployeeId,
                                                Arrays.asList(leaveDateEntity)));

                assertTrue(exception.getMessage().contains("Conflict") ||
                                exception.getMessage().contains("PRESENT"));
        }
        ...
        void testHandleTimesheetUpdateApproval_LateToOnTime_UpdateMetrics() {
                // Arrange - Date with late check-in (2024-12-02 from sample data has 60 late
                // minutes)
        ...
        // ============= Extended handleTimesheetUpdateApproval() Tests =============

        @Test
        @DisplayName("Should update timesheet with new morning status")
        void testHandleTimesheetUpdateApproval_Extended_UpdateMorningStatus() {
                // Arrange - Use existing timesheet from sample data (2024-12-03 has no specific
                // status set)
                LocalDate targetDate = LocalDate.of(2024, 12, 3);

                // Act - Update morning status to LEAVE (extended method)
                DailyTimeSheet result = timeSheetCommandService.handleTimesheetUpdateApproval(
                                testEmployeeId,
                                targetDate,
                                null, // No check-in change
                                null, // No check-out change
                                AttendanceStatus.LEAVE, // Set morning to LEAVE
                                null, // No afternoon change
                                null, // No morning WFH change
                                null // No afternoon WFH change
                );
        ...
        @Test
        @DisplayName("Should update timesheet with WFH flags for full day")
        void testHandleTimesheetUpdateApproval_Extended_SetWfhFlags() {
                // Arrange - Date with existing timesheet that is not finalized
                LocalDate targetDate = LocalDate.of(2024, 12, 3);

                // Act - Set full day WFH
                DailyTimeSheet result = timeSheetCommandService.handleTimesheetUpdateApproval(
                                testEmployeeId,
                                targetDate,
                                null, // No check-in change
                                null, // No check-out change
                                AttendanceStatus.PRESENT, // Morning PRESENT for WFH
                                AttendanceStatus.PRESENT, // Afternoon PRESENT for WFH
                                true, // Morning WFH
                                true // Afternoon WFH
                );
                ...
                assertEquals(AttendanceStatus.PRESENT, result.getMorningStatus());
                assertEquals(AttendanceStatus.PRESENT, result.getAfternoonStatus());
                assertEquals(1.0, result.getTotalWorkCredit(), 0.001); // Full day credit for WFH
        }

        @Test
        @DisplayName("Should update timesheet with half-day WFH and half-day leave")
        void testHandleTimesheetUpdateApproval_Extended_HalfWfhHalfLeave() {
                // Arrange
                LocalDate targetDate = LocalDate.of(2024, 12, 3);

                // Act - Morning WFH, afternoon leave
                DailyTimeSheet result = timeSheetCommandService.handleTimesheetUpdateApproval(
                                testEmployeeId,
                                targetDate,
                                null,
                                null,
                                AttendanceStatus.PRESENT, // Morning PRESENT (WFH)
                                AttendanceStatus.LEAVE, // Afternoon LEAVE
                                true, // Morning WFH
                                false // Afternoon not WFH
                );
                ...
                assertEquals(AttendanceStatus.PRESENT, result.getMorningStatus());
                assertEquals(AttendanceStatus.LEAVE, result.getAfternoonStatus());
                assertEquals(0.5, result.getTotalWorkCredit(), 0.001); // Half day credit
        }

        @Test
        @DisplayName("Should update timesheet with both times and status changes")
        void testHandleTimesheetUpdateApproval_Extended_TimesAndStatus() {
                // Arrange - Update with new times and set statuses
                LocalDate targetDate = LocalDate.of(2024, 12, 2); // Has late minutes in sample data
                LocalDateTime newCheckIn = LocalDateTime.of(2024, 12, 2, 8, 0); // On time
                LocalDateTime newCheckOut = LocalDateTime.of(2024, 12, 2, 17, 0); // Standard time
                ...
                assertEquals(newCheckIn, result.getCheckInTime());
                assertEquals(newCheckOut, result.getCheckOutTime());
                assertEquals(0, result.getLateMinutes()); // Now on time
                assertEquals(0, result.getEarlyLeaveMinutes());
                assertEquals(AttendanceStatus.PRESENT, result.getMorningStatus());
                assertEquals(AttendanceStatus.PRESENT, result.getAfternoonStatus());
                // 9 hours work = 1.0 credit (time-based calculation since not WFH)
                assertEquals(1.0, result.getTotalWorkCredit(), 0.001);
        }

        @Test
        @DisplayName("Should calculate correct work credit for full day leave via extended method")
        void testHandleTimesheetUpdateApproval_Extended_FullDayLeave() {
                // Arrange
                LocalDate targetDate = LocalDate.of(2024, 12, 3);

                // Act - Set full day leave (no times, no WFH)
                DailyTimeSheet result = timeSheetCommandService.handleTimesheetUpdateApproval(
                                testEmployeeId,
                                targetDate,
                                null, // No check-in
                ...
                assertEquals(AttendanceStatus.LEAVE, result.getMorningStatus());
                assertEquals(AttendanceStatus.LEAVE, result.getAfternoonStatus());
                assertEquals(0.0, result.getTotalWorkCredit(), 0.001); // Leave = 0 credit
        }

        @Test
        @DisplayName("Should throw exception for extended update on finalized timesheet")
        void testHandleTimesheetUpdateApproval_Extended_Finalized_ThrowsException() {
                // Arrange - Finalized timesheet from sample data (2024-12-05)
                LocalDate finalizedDate = LocalDate.of(2024, 12, 5);
        ...
        @Test
        @DisplayName("Should preserve existing values when null parameters passed")
        void testHandleTimesheetUpdateApproval_Extended_PreserveExistingValues() {
                // Arrange - Get current state of timesheet (2024-12-01)
                LocalDate targetDate = LocalDate.of(2024, 12, 1);
                DailyTimeSheet original = dailyTimeSheetDao.getTimesheetByEmployeeAndDate(
                                testEmployeeId, targetDate);
                assertNotNull(original);
                LocalDateTime originalCheckIn = original.getCheckInTime();
                LocalDateTime originalCheckOut = original.getCheckOutTime();

                // Act - Update only WFH flags, keep everything else null
                DailyTimeSheet result = timeSheetCommandService.handleTimesheetUpdateApproval(
                                testEmployeeId,
                                targetDate,
                                null, // Keep existing check-in
                                null, // Keep existing check-out
                                null, // Keep existing morning status
                                null, // Keep existing afternoon status
                                true, // Only set morning WFH
                                null // Keep existing afternoon WFH
                );

                // Assert - Original times should be preserved
                assertEquals(originalCheckIn, result.getCheckInTime());
                assertEquals(originalCheckOut, result.getCheckOutTime());
                assertTrue(result.getMorningWfh()); // Updated
        }
}
